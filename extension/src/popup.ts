  ...
  contentArea.innerHTML = `
    <div class="glass-card w-full p-4 flex flex-col items-center animate-[fade-in_0.3s_ease-out]">
      <div class="relative mb-3">
        ...
        <img src="${data.photoUrl || 'https://via.placeholder.com/150'}" alt="Profile" class="w-24 h-24 rounded-full border-2 border-gold relative z-10 object-cover" />
      </div>
      ...
      <p class="text-[11px] text-slate-400 text-center mb-4">${data.company} • ${data.location}</p>
      ...
      <div class="w-full space-y-2 mt-2">
        ...
        <div class="flex items-center gap-3 bg-slate-800/60 p-2 rounded-md border border-slate-700/50">
          ...
          <span class="text-xs text-slate-300">${data.connectionCount} connections</span>
        </div>` : ''}
      </div>
    </div>
  `;
